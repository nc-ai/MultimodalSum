...
def sort_yelp_data(path):
    ...
    for line in lines:
        ...
        bi = line_dict['business_id']

        if bi not in all_reviews:
            all_reviews[bi] = list()

        all_reviews[bi].append(line)

    with open(path, 'wt') as f:
        for review_list in all_reviews.values():
...
def read_yelp_data(path):
    ...
    yelp_to_output_map = {
        ...
        YelpFields.REV_ID: OutputFields.REV_ID,
...
def clean_text(text_str):
    return_str = text_str.replace("\t", ' ').replace('\n', ' ')

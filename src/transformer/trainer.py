...
class Trainer:
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if epoch * (len(train_dataloader) // self.args.gradient_accumulation_steps) + step >= off_step:
                    tr_loss += self.training_step(model, inputs, off_dropout=True)
                else:
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], off_dropout=False) -> float:
        """
